...
class CarManager {
    ...
    // Get mobil terbaru - PERBAIKAN: include foto_mobil
    ...
    // PERBAIKAN: Add mobil baru dengan foto_mobil
    public function addCar($penjual_id, $plat_mobil, $no_mesin, $rangka_mesin, $slinder, $merk, $model, $tahun, $warna, $bahan_bakar, $transmisi, $kilometer, $harga, $deskripsi, $foto_mobil = null) {
        ...
        // PERBAIKAN: Tambahkan parameter foto_mobil
        if ($foto_mobil) {
            $sql = "INSERT INTO mobil (penjual_id, plat_mobil, no_mesin, rangka_mesin, slinder, merk, model, tahun, warna, bahan_bakar, transmisi, kilometer, harga, deskripsi, foto_mobil) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            
            $params = [
                $penjual_id, $plat_mobil, $no_mesin, $rangka_mesin, $slinder, 
                $merk, $model, $tahun, $warna, $bahan_bakar, $transmisi, 
                $kilometer, $harga, $deskripsi, $foto_mobil
            ];
        } else {
    ...
    // PERBAIKAN: Add mobil baru dengan data array termasuk foto_mobil
    public function addCarWithData($data) {
        return $this->addCar(
            ...
            $data['harga'],
            $data['deskripsi'] ?? '',
            $data['foto_mobil'] ?? null  // PERBAIKAN: Tambah parameter foto_mobil
        );
    }
    
    // PERBAIKAN: Update mobil dengan foto_mobil
    public function updateCar($id, $penjual_id, $data) {
        $allowed_fields = ['plat_mobil', 'no_mesin', 'rangka_mesin', 'slinder', 'merk', 'model', 'tahun', 'warna', 'bahan_bakar', 'transmisi', 'kilometer', 'harga', 'deskripsi', 'status', 'foto_mobil']; // PERBAIKAN: tambah foto_mobil
        $updates = [];
        $params = [];
    ...
    // Upload foto mobil (untuk tabel foto_mobil)
    ...
    // Get foto mobil (dari tabel foto_mobil)
    ...
    // PERBAIKAN: Update foto mobil utama
    public function updateCarPhoto($id, $penjual_id, $foto_mobil) {
        $sql = "UPDATE mobil SET foto_mobil = ? WHERE id = ? AND penjual_id = ?";
        try {
            $stmt = $this->pdo->prepare($sql);
            return $stmt->execute([$foto_mobil, $id, $penjual_id]);
        } catch(PDOException $e) {
            error_log("Error updateCarPhoto: " . $e->getMessage());
            return false;
        }
    }
    
    // PERBAIKAN: Hapus foto mobil utama
    public function deleteCarPhoto($id, $penjual_id) {
        // Ambil nama file foto sebelum dihapus
        $sql = "SELECT foto_mobil FROM mobil WHERE id = ? AND penjual_id = ?";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$id, $penjual_id]);
        $car = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if ($car && $car['foto_mobil']) {
            // Hapus file fisik
            $file_path = '../uploads/cars/' . $car['foto_mobil'];
        ...
        // Update database
        $sql = "UPDATE mobil SET foto_mobil = NULL WHERE id = ? AND penjual_id = ?";
        $stmt = $this->pdo->prepare($sql);
